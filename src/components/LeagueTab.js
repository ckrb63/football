...
const LeagueTab = (props) => {
  const [selectedLeague, setSelectedLeague] = useState('E');
  ...
  return <div className={style.leaguetab}>
    <div id={39} onClick={onClickHandlerPremeir} className={selectedLeague==='E'?`${style.league} ${style.active}`:style['league']}>
      <img src={PREMIER_LEAGUE_LOGO} alt='PREMIER_LEAGUE'></img>
    </div>
    <div id={140} onClick={onClickHandlerLaliga} className={selectedLeague==='L'?`${style.league} ${style.laliga} ${style.active}`:`${style.league} ${style.laliga}`}>
      <img src={LALIGA_LOGO} alt='LALIGA'></img>
    </div>
    <div id={78} onClick={onClickHandlerBund} className={selectedLeague==='B'?`${style.league} ${style.active}`:style['league']}>
      <img src={BUNDESLIGA_LOGO} alt='BUNDESLIGA'></img>
    </div>
    <div id={135} onClick={onClickHandlerSerie} className={selectedLeague==='I'?`${style.league} ${style.active}`:style['league']}>
      <img src={SERIE_A_LOGO} alt='SERIE_A'></img>
    </div>
    <div id={61} onClick={onClickHandlerLigue} className={selectedLeague==='F'?`${style.league} ${style.league1} ${style.active}`:`${style.league} ${style.league1}`}>
